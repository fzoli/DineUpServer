...
import com.dineup.api.service.impl.v1_0.element.ProfileElement;
...
public class Utils {
    ...
    public static void completeProfileElement(ProfileElement profile, ApiConfig apiConfig) {
        if (profile == null) {
            return;
        }
        String photoUrl = profile.photoUrl;
        if (photoUrl != null && photoUrl.startsWith("/")) {
            profile.photoUrl = Strings.concat("/", apiConfig.getTarget().getServerUrl(), photoUrl);
        }
    }
